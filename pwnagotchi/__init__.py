...
def pwnagotchi_cli():
    ...
    if args.check_update:
        ...
        if remote > local:
            ...
            print("You are currently on the latest release, v%s." % __version__)
        sys.exit(0)
    ...
    _config = config
    fs.setup_mounts(_config)
    log.setup_logging(args, _config)
    fonts.init(_config)

    set_name(_config['main']['name'])

    plugins.load(_config)

    display = Display(config=_config, state={'name': '%s>' % name()})

    if args.do_clear:
        do_clear(display)
        sys.exit(0)

    agent = Agent(view=display, config=_config, keypair=KeyPair(view=display))
