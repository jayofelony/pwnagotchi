...
class FixServices(plugins.Plugin):
    ...
    def _tryTurningItOffAndOnAgain(self, connection):
        ...
        # monstop/monstart go through pwnlib's stop_monitor_interface /
        # start_monitor_interface, which already retry the brcmfmac module
        # reload and, if the SDIO backplane itself has wedged, reboot as a
        # last resort - no need to duplicate that modprobe-cycling logic
        # here anymore.
        ...
        try:
            ...
            try:
                ...
                if "success" not in result:
                    logging.debug("[Fix_Services] set wifi.interface wlan0mon failed? %s" % repr(result))
            except Exception as err:
                logging.debug("[Fix_Services] set wifi.interface wlan0mon except: %s" % repr(err))
        except Exception as err:
            logging.error("[Fix_Services] wlan0mon still missing after monstart: %s" % repr(err))
            if display:
                display.update(force=True, new_data={"status": "Still blind...", "face": faces.BROKEN})

        time.sleep(10)  # give it a bit before restarting recon in bettercap
        self.isReloadingMon = False
