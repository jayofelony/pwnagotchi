...
class ohcapi(plugins.Plugin):
    ...
    def on_config_changed(self, config):
        ...
        self.sleep = self.options.get("sleep", 60 * 60)  # 1 hour
        self.ready = True
            ...
            if handshake_new:
                ...
                for idx, pcap_path in enumerate(handshake_new):
                    ...
                    if hashes:
                        ...
                        logging.debug(
                            f"OHC NewAPI: No hashes extracted from {pcap_path}, skipping."
                        )
                        self.skip.append(pcap_path)
                ...
                if all_hashes:
                    ...
                    for batch_idx, batch in enumerate(batches):
                        display.on_uploading(
                            f"onlinehashcrack.com ({(batch_idx+1)*50}/{len(all_hashes)})"
                        )
